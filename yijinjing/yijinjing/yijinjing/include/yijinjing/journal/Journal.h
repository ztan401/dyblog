...
inline void *Journal::locateFrame() {
    ...
    if (isWriting) {
        ...
        if ((curPage.get() != nullptr && curPage->isAtPageEnd()) || curPage.get() == nullptr)
            loadNextPage();
        if (curPage.get() != nullptr) return curPage->locateReadableFrame();
    }
    return nullptr;
};

inline void Journal::passFrame() {// only called after frame is taken, so current frame is
                                  // applicable for sure; just skip
    curPage->passFrame();
}

inline short Journal::getCurPageNum() const { return curPage->getPageNum(); }

inline string Journal::getShortName() const { return shortName; }
