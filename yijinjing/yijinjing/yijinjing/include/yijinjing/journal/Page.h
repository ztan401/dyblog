...
private:
    /** internal usage */
    inline FH_TYPE_STATUS getCurStatus() const { return frame.getStatus(); }
...
public:
   ...
   * will not lock memory if in quickMode (locked by page engine service)*/
    static PagePtr load(const string &dir, const string &jname, short pageNum, bool isWriting,
                        bool quickMode);
};

inline bool Page::isAtPageEnd() const { return getCurStatus() == JOURNAL_FRAME_STATUS_PAGE_END; }
...
inline void Page::passWrittenFrame() {
    while (getCurStatus() == JOURNAL_FRAME_STATUS_WRITTEN) passFrame();
}

inline void Page::passToTime(int64_t time) {
    while (getCurStatus() == JOURNAL_FRAME_STATUS_WRITTEN && frame.getNano() < time) passFrame();
}
