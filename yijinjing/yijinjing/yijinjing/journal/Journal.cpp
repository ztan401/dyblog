...
#include "yijinjing/journal/Journal.h"
...
bool Journal::seekTime(int64_t time) {
    ...
    if (time == TIME_TO_LAST) {
        ...
        curPage = pageProvider->getPage(directory, shortName, serviceIdx,
                                        (pageNums.size() > 0) ? pageNums.back() : 1);
        if (curPage.get() != nullptr) curPage->passWrittenFrame();
    } else if (time == TIME_FROM_FIRST) {
        const vector<short> pageNums = PageUtil::GetPageNums(directory, shortName);
        curPage = pageProvider->getPage(directory, shortName, serviceIdx,
                                        (pageNums.size() > 0) ? pageNums.front() : 1);
    } else {
        ...
        if (curPage.get() != nullptr) curPage->passToTime(time);
    }
...
void Journal::loadNextPage() {
    ...
    if (curPage.get() == nullptr) {
        ...
        PagePtr newPage =
                pageProvider->getPage(directory, shortName, serviceIdx, curPage->getPageNum() + 1);
        // stop current page
        if (isWriting) {
            curPage->finishPage();
        }
