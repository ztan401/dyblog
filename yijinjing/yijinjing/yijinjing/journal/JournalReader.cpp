...
JournalReader::JournalReader(PageProviderPtr &ptr) : JournalHandler(ptr) { journalMap.clear(); }
...
JournalReaderPtr JournalReader::create(const vector<string> &dirs, const vector<string> &jnames,
                                       int64_t startTime, const string &readerClientName) {
    ...
    for (size_t i = 0; i < dirs.size(); i++) jrp->addJournal(dirs[i], jnames[i]);
...
void JournalReader::jumpStart(int64_t startTime) {
    for (JournalPtr &journal: journals) journal->seekTime(startTime);
}
...
void JournalReader::startVisiting() {
    ...
    while (true) {
        ...
        if (frame.get() != nullptr) {
            ...
            for (auto visitor: visitors) visitor->visit(name, *frame);
        }
    }
}
...
bool JournalReader::expireJournalByName(const string &jname) {
    ...
    if (iter == journalMap.end()) return false;
